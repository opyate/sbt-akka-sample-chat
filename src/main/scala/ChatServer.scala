...
class ChatClient(val name: String) {
  val chat = RemoteClient.actorFor("chat:service", "localhost", 9998)

  def login                 = chat ! Login(name)
  def logout                = chat ! Logout(name)
  def post(message: String) = chat ! ChatMessage(name, name + ": " + message)
  def chatLog               = {
    (chat !! GetChatLog(name)) match {
      case Some(cl) => cl.asInstanceOf[ChatLog]
      case None => throw new Exception("Couldn't get the chat log from ChatServer")
    }
  }
  // orElse (throw new Exception("Couldn't get the chat log from ChatServer"))
}
